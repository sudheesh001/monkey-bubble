...
static void
mb_net_connection_init (MbNetConnection * self)
{
  Private *priv;
...
static void
mb_net_connection_finalize (MbNetConnection * self)
{
  Private *priv;
  priv = GET_PRIVATE (self);
...
void
mb_net_connection_close (MbNetConnection * self, GError ** error)
{
  Private *priv;
  priv = GET_PRIVATE (self);
  if (priv->socket != -1)
...
void
mb_net_connection_stop (MbNetConnection * self, GError ** error)
{
  Private *priv;
  GError *err;
  err = NULL;
			   ...
			   GError ** error)
{
  ...
  if (sock == -1)
    {
      ...
      g_set_error (error, error_quark,
		   MB_NET_CONNECTION_SOCKET_ERROR, "unable to create socket");
      return;
    }
  ...
  if (!src_host)
    {
      ...
      g_set_error (error, error_quark,
		   MB_NET_CONNECTION_SOCKET_ERROR, "invalide name");
      close (sock);
      return;
    }
  ...
  while (connect
	 (sock, (struct sockaddr *) &sock_client, sizeof (sock_client)) == -1)
    {
      if (errno != EAGAIN)
	{
			...
			GError ** error)
{
  ...
  if (bind (sock,
	    (struct sockaddr *) &sock_client, sizeof (sock_client)) == -1)
    {

      close (sock);
      perror ("bind server socket error ");
      g_set_error (error, error_quark,
		   MB_NET_CONNECTION_SOCKET_ERROR, "unable to bind socket");
      return -1;
    }
  ...
  if (listen (sock, MAX_WAITING_CONN) == -1)
    {
      ...
      g_set_error (error, error_quark,
		   MB_NET_CONNECTION_SOCKET_ERROR, "unable to listen socket");
      return -1;
    }
  return sock;
}
...
void *
_accept_loop (MbNetConnection * self)
{
  ...
  while (priv->stop == FALSE && priv->socket != -1)
	...
	{
	  sock = accept (ssock, (struct sockaddr *) &sock_client, &lg_info);
	  ...
	  if (sock != -1)
		...
		MB_NET_CONNECTION (g_object_new
				   (MB_NET_TYPE_CONNECTION, NULL));
	      GET_PRIVATE (con)->socket = sock;
	      g_signal_emit (self, _signals[NEW_CONNECTION], 0, con);
	      g_object_unref (con);
...
void
_read_message (MbNetConnection * self)
{
  Private *priv;
  guint32 size;
  priv = GET_PRIVATE (self);
  ...
  if (read (priv->socket, &size, sizeof (size)) < 1)
	...
	{
	  if ((readed = read (priv->socket, data + p, size)) < 1)
	    {
	      perror ("read()");
	      g_free (data);
	      return;
...
void *
_listen_loop (MbNetConnection * self)
{
...
void
mb_net_connection_listen (MbNetConnection * self, GError ** error)
{
  Private *priv;
  GError *err;
				...
				MbNetMessage * message, GError ** error)
{
  _mb_net_connection_send_message (self, mb_net_message_size (message),
				   mb_net_message_data (message), error);
}
...
static void
mb_net_connection_class_init (MbNetConnectionClass * mb_net_connection_class)
{
  ...
  g_object_class->finalize = (GObjectFinalizeFunc) mb_net_connection_finalize;
  ...
  _signals[NEW_CONNECTION] = g_signal_new ("new-connection",
					   MB_NET_TYPE_CONNECTION,
					   G_SIGNAL_RUN_LAST,
					   G_STRUCT_OFFSET
					   (MbNetConnectionClass,
  ...
  error_quark = g_quark_from_static_string ("MB_NET_CONNECTION_ERROR");
}
